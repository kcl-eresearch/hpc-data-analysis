...
OUTPUT_FILE = SCRIPT_DIR / "output_job_structure.txt"
...
with open(OUTPUT_FILE, 'w') as f:
    ...
    # =============================================================================
    out("=" * 80)
    out("PART 1: Sample job with all relevant columns (create_job_table)")
    out("=" * 80)
...
""")
    ...
    # =============================================================================
    out("=" * 80)
    out("PART 2: Steps for the sample job (create_step_table)")
    out("=" * 80)
    ...
    if sample_job_db_inx:
        ...
        for row in rows:
            out(f"""  Step id_step={row[0]}, name='{row[1]}':
    state:             {row[2]}
    time_start:        {row[3]}
    time_end:          {row[4]}
    user_sec:          {row[5]}
...
""")
    ...
    # =============================================================================
    out("=" * 80)
    out("PART 3: How many steps do jobs typically have?")
    out("=" * 80)
    ...
    out(f"  {'-'*6}  {'-'*12}")
    for row in cur:
        out(f"  {row[0]:>6}  {row[1]:>12}")

    # =============================================================================
    # Part 4: Sample job with SINGLE step
    # =============================================================================
    out("\n" + "=" * 80)
    out("PART 4: Sample job with SINGLE step")
    out("=" * 80)
...
""")
    ...
    # =============================================================================
    out("=" * 80)
    out("PART 5: Sample job with MULTIPLE steps")
    out("=" * 80)
...
Job {id_job} (multiple steps):
  ...
  Steps:""")
        ...
        out(f"    {'id_step':>8}  {'step_name':<12}  {'user_sec':>10}  {'sys_sec':>10}  {'user_usec':>10}  {'sys_usec':>10}  tres_usage_in_max")
        out(f"    {'-'*8}  {'-'*12}  {'-'*10}  {'-'*10}  {'-'*10}  {'-'*10}  {'-'*20}")
        for step in cur:
            out(f"    {step[0]:>8}  {str(step[1]):<12}  {step[2]:>10}  {step[3]:>10}  {step[4]:>10}  {step[5]:>10}  {step[6]}")

    # =============================================================================
    # Part 6: What step names exist?
    # =============================================================================
    out("\n" + "=" * 80)
    out("PART 6: Step names and their frequencies")
    out("=" * 80)
    ...
    out(f"  {'-'*20}  {'-'*8}  {'-'*12}")
    for row in cur:
        out(f"  {str(row[0]):<20}  {row[1]:>8}  {row[2]:>12}")
